...
file_path="ZRDN3/detected_Zrdn3.txt" # Обнаруженные цели ЗРДН.
file2_path="ZRDN3/confrimed_balistic_Zrdn3.txt" # Подтвержденные цели ЗРДН.
file3_path="ZRDN3/shot_Zrdn3.txt" # Цели, по которым произведен выстрел.
...
echo "$time,Zrdn3,i am alive" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "messages/alive/$time.log"
...
if [ -d "$directory" ]; then # Является ли путь директорией
    ...
    while true; do
        ...
        for name in "${filenames[@]}"; do
            ...
            y="${y#Y}"
            circle=$(target_in_circle $x $y $ZRDN3_x $ZRDN3_y $ZRDN3_RADIUS) # Находится ли цель в пределах области действия ЗРДН.
            if [ "$circle" -eq 1 ]; then # Цель находится в пределах области действия ЗРДН.
                if ! grep -q "^$id " "$file2_path"; then # Если цель записана в файле confrimed_balistic_Zrdn1, то мы её игнорируем.
                    if grep -q "^$id " "$file3_path"; then # Был сделан выстрел, но цель не была уничтожена.
                        ...
                        echo "$time,Zrdn3,no kill,$id,$x $y" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                        ...
                        if ! grep -q "^($id " "$file3_path"; then
                            if grep -q "^$id " "$file_path"; then # Цель была обнаружена и ее координаты записаны в файл.
                                ...
                                if [[ -n $speed_info && $speed_info != 0 ]]; then # Есть ли скорость, неравная нулю.
                                    ...
                                    echo "$time,Zrdn3,confirm,$id,$x $y,$speed_info,$type" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                                    if [[ $rocket -gt 0 && ($type == "rocket" || $type == "plane") ]]; then # Есть ракеты и цель является ракетой или самолетом.
                                        ...
                                        echo "$time,Zrdn3,shot,$id,$x $y,$speed_info,$type" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                                        echo "$time,Zrdn3,ammo left,$rocket" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/-$time.log"
                                    ...
                                    if [[ $rocket == 0 ]]; then
                                        ...
                                        echo "$time,Zrdn3,no ammo" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                                        rocket=$(( rocket - 1 ))
                                    fi
                                    speed_info=0
                                fi
        ...
        while IFS= read -r line; do
            ...
            echo "$time,Zrdn3,destroyed,$line" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
        ...
        if [ -f "ZRDN3/hello.log" ]; then
            decrypted_content=$(openssl aes-256-cbc -pbkdf2 -a -d -salt -pass "pass:$password" -in "ZRDN3/hello.log")
