...
file_path="ZRDN3/detected_Zrdn3.txt"
file2_path="ZRDN3/confrimed_balistic_Zrdn3.txt"
file3_path="ZRDN3/shot_Zrdn3.txt"
...
if [ -d "$directory" ]; then
    ...
    while true; do
        ...
        for name in "${filenames[@]}"; do
            ...
            x="${x#X}"
            y="${y#Y}"
            circle=$(target_in_circle $x $y $ZRDN3_x $ZRDN3_y $ZRDN3_RADIUS)
            if [ "$circle" -eq 1 ]; then
                    ...
                    if ! grep -q "^$id " "$file3_path"; then
                        if ! grep -q "^($id " "$file3_path"; then
                            if grep -q "^$id " "$file_path"; then
                                ...
                                #echo "$time,Zrdn3,detect,$id,$x $y" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/Zrdn3-$id-detect-$time.log"
                            fi
                        fi
                    fi
                fi
