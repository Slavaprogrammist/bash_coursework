...
file_path="ZRDN2/detected_Zrdn2.txt" # Обнаруженные цели ЗРДН.
file2_path="ZRDN2/confrimed_balistic_Zrdn2.txt" # Подтвержденные цели ЗРДН.
file3_path="ZRDN2/shot_Zrdn2.txt" # Цели, по которым произведен выстрел.
...
if [ -d "$directory" ]; then # Является ли путь директорией
    ...
    while true; do
        ...
        for name in "${filenames[@]}"; do
            ...
            y="${y#Y}"
            circle=$(target_in_circle $x $y $ZRDN2_x $ZRDN2_y $ZRDN2_RADIUS) # Находится ли цель в пределах области действия ЗРДН.
