...
file_path="RLS2/detected_Rls2.txt"
file2_path="RLS2/confrimed_Rls2.txt"
...
if [ -d "$directory" ]; then
   ...
   while true; do
      ...
      for name in "${filenames[@]}"; do
         ...
         x="${x#X}"
         y="${y#Y}"
         sector=$(target_in_sector $x $y $RLS2_x $RLS2_y $RLS2_RADIUS $RLS2_START_ANGLE $RLS2_END_ANGLE)
         if [ "$sector" -eq 1 ]; then
            ...
            if ! grep -q "^$id " "$file2_path"; then
               ...
               if grep -q "^$id " "$file_path"; then
                  ...
                  if [[ -n $speed_info && $speed_info != 0 ]]; then
                     ...
                     echo "$time,Rls2,confirm,$id,$x $y,$speed_info,$type" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                     ...
                     if [[ $to_spro == "true" && $type == "ballistic" && $(echo "${distance1} > ${distance2}" | bc -l) ]]; then
                        ...
                        echo "$time,Rls2,toSpro,$id,$x $y" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                     fi
                     speed_info=0
                  fi
               else
                  ...
                  #echo "$time,Rls2,detect,$id,$x $y" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/Rls2-$id-detect-$time.log"
               fi
            fi
         fi
      done
