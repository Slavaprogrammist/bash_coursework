...
file_path="RLS1/detected_Rls1.txt"
file2_path="RLS1/confrimed_Rls1.txt"
...
if [ -d "$directory" ]; then
   ...
   while true; do
      ...
      for name in "${filenames[@]}"; do
         ...
         x="${x#X}"
         y="${y#Y}"
         sector=$(target_in_sector $x $y $RLS1_x $RLS1_y $RLS1_RADIUS $RLS1_START_ANGLE $RLS1_END_ANGLE)
         if [ "$sector" -eq 1 ]; then
            ...
            if ! grep -q "^$id " "$file2_path"; then
               ...
               if grep -q "^$id " "$file_path"; then
                  ...
                  if [[ -n $speed_info && $speed_info != 0 ]]; then
                     ...
                     echo "$time,Rls1,confirm,$id,$x $y,$speed_info,$type" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                     ...
                     if [[ $to_spro == "true" && $type == "ballistic" && $(echo "${distance1} > ${distance2}" | bc -l) ]]; then
                        time=$(TZ=Europe/Moscow date +"$time_format")
                        echo "$time, Target id $id X $x Y $y goes to Spro"
                        echo "$time,Rls1,toSpro,$id,$x $y" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                     fi
                     speed_info=0
                  fi
               else
                  ...
                  #echo "$time,Rls1,detect,$id,$x $y" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/Rls1-$id-detect-$time.log"
               fi
            fi
         fi
      done
      if [ -f "RLS1/hello.log" ]; then
