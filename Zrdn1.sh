...
file_path="ZRDN1/detected_Zrdn1.txt"
file2_path="ZRDN1/confrimed_balistic_Zrdn1.txt"
file3_path="ZRDN1/shot_Zrdn1.txt"
...
if [ -d "$directory" ]; then
    ...
    while true; do
        ...
        for name in "${filenames[@]}"; do
            ...
            x="${x#X}"
            y="${y#Y}"
            circle=$(target_in_circle $x $y $ZRDN1_x $ZRDN1_y $ZRDN1_RADIUS)
            if [ "$circle" -eq 1 ]; then
                    ...
                    if ! grep -q "^$id " "$file3_path"; then
                        if ! grep -q "^($id " "$file3_path"; then
                            if grep -q "^$id " "$file_path"; then
                                ...
                                #echo "$time,Zrdn1,detect,$id,$x $y" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/Zrdn1-$id-detect-$time.log"
                            fi
                        fi
                    fi
                fi
